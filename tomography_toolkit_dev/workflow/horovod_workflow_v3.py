...
class hvd_workflow_res(Workflow):
    """
    ...
    def fit(self):
        ...
        #     torch.optim.Adam(self.discriminator.parameters(),lr=self.discriminator_lr*self.n_workers,betas=(0.5,0.999)),
        #     backward_passes_per_step=self.num_backward_passes,
        ...
        for epoch in range(self.start_epoch, 1 + self.num_epochs):
            ...
            if self.snapshot_epoch > 0 and (epoch % self.snapshot_epoch == 0): #and hvd.rank() == 0:
                epoch_str = str(epoch) + 'epochs' + '_rank' + str(hvd.rank())
        ...
        if hvd.rank() != -1:
            epoch_str = str(self.num_epochs) + 'epochs' + '_rank' + str(hvd.rank())
            self.write_model_to_file(self.generator, self.model_dir + '/generator_' + epoch_str + '.pt')
            self.write_model_to_file(self.discriminator, self.model_dir + '/discriminator_' + epoch_str + '.pt')
            ...
            # Accumulated losses:
            self.hvd_disc_real_loss = torch.as_tensor(loss_accuracy_dict[
                                                          'disc_real_loss'])  # hvd.allreduce(torch.as_tensor(loss_accuracy_dict['disc_real_loss']),'disc_real_loss')
            self.hvd_disc_fake_loss = torch.as_tensor(loss_accuracy_dict[
                                                          'disc_fake_loss'])  # hvd.allreduce(torch.as_tensor(loss_accuracy_dict['disc_fake_loss']),'disc_fake_loss')
            self.hvd_gen_loss = torch.as_tensor(loss_accuracy_dict[
                                                    'gen_loss'])  # hvd.allreduce(torch.as_tensor(loss_accuracy_dict['gen_loss']),'gen_loss')

            # Accumulated accuracies:
            self.hvd_real_acc = torch.as_tensor(loss_accuracy_dict[
                                                    'disc_real_acc'])  # hvd.allreduce(torch.as_tensor(loss_accuracy_dict['disc_real_acc']),'disc_real_acc')
            self.hvd_fake_acc = torch.as_tensor(loss_accuracy_dict[
                                                    'disc_fake_acc'])  # hvd.allreduce(torch.as_tensor(loss_accuracy_dict['disc_fake_acc']),'disc_fake_acc')

            # Handle times:
            self.hvd_training_time = torch.as_tensor(
                training_time)  # hvd.allreduce(torch.as_tensor(training_time),'avg_training_time')
    ...
    def load_model_weights(self, current_epoch, rank):
        epoch_str = str(current_epoch) + 'epochs' + '_rank' + str(rank)
        ...
        for epoch in range(self.start_epoch, 1 + self.num_epochs):
            if epoch % self.snapshot_epoch == 0:
                for rank in range(self.n_workers):
                    epoch_str = self.load_model_weights(epoch, rank)
