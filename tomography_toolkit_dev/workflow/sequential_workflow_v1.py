...
class seq_ae_workflow(Workflow):
    """
    ...
    def __init__(self, module_names,
        ...
        self.name = "sequential_workflow_v1"
    ...
    def load_tools(self):
        ...
        self.performance_monitor = AE_Performance_Monitor(
    ...
    #*****************************************************
    def fit(self):
        # Set up optimizers for the encoder:
        generator_optimizer = torch.optim.Adam(self.generator.parameters(),lr=self.generator_lr,betas=(0.5,0.999))
        ...
        for epoch in range(1,1+self.num_epochs):
            ...
            self.performance_monitor.watch_loss_and_dropout_rate_per_batch(
            ...
            if epoch % self.read_performance_epoch == 0:
                self.performance_monitor.collect_loss_and_dropout_rate_per_epoch()
            ...
            if epoch % self.print_info_epoch == 0:
                ...
                self.performance_monitor.print_loss_and_dropout_rate()
                print(" ")
            #----------------------------------------------------
        ...
        loss_dropout_rate_dict = self.performance_monitor.read_out_loss_and_dropout_rate()

        # Accumulated loss and dropout rate:
        self.hvd_gen_loss = loss_dropout_rate_dict['gen_loss']
        self.hvd_dropout_rate = loss_dropout_rate_dict['dropout_rate']

        # Handle times:
        self.hvd_training_time = training_time

        # Get the gradients:
        if self.watch_gradients == True:
            gen_gradients = self.gen_grad_mon.read_out_gradients()
            
            self.hvd_gen_min_grad = gen_gradients['minimum_gradients']
            self.hvd_gen_max_grad = gen_gradients['maximum_gradients']
            self.hvd_gen_avg_grad = gen_gradients['average_gradients']
    #*****************************************************
    ...
    def snaphsot_analysis(self):
        ...
        for epoch in range(1,1+self.num_epochs):
            if epoch % self.snapshot_epoch == 0:
                ...
                current_params, current_gen_data, current_residuals = self.performance_monitor.predict_observables_and_residuals(real_data=monitoring_data_chunk)
    ...
    def post_training_analysis(self,n_samples):
        ...
        # Plot losses and accuracy as a function of the training epoch:
          loss_plots = self.performance_plotter.plot_loss_and_dropout_rate(
             gen_loss=self.hvd_gen_loss, #--> Generator loss,
             dropout_rate=self.hvd_dropout_rate, #--> Dropout rate
          ...
          axt.plot(self.hvd_training_time,self.hvd_gen_loss,'b-',linewidth=2.0)
          ...
          if self.store_results_as_npy == True:
            ...
            np.save(self.data_science_npy_dir + '/generator_loss.npy',self.hvd_gen_loss)
            np.save(self.data_science_npy_dir + '/dropout_rate.npy',self.hvd_dropout_rate)
          ...
          if self.watch_gradients == True:
            ...
            gen_gradients = {
                'average_gradients': self.hvd_gen_avg_grad,
                'minimum_gradients': self.hvd_gen_min_grad,
                'maximum_gradients': self.hvd_gen_max_grad
            }
