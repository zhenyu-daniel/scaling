...
class hvd_workflow(Workflow):
    ...
    def __init__(self, module_names,
        ...
        workflow_parser.add_argument('--num_backward_passes',type=int,default=self.read_settings_from_config("num_backward_passes",training_config,1),metavar='N',help='Number of backward passes before updating')
        ...
        self.g_workers = self.n_workers-1
        self.d_workers = self.n_workers- self.g_workers
    ...
    def fit(self):
        ...
        generator_optimizer = hvd.DistributedOptimizer(
            torch.optim.Adam(self.generator.parameters(),lr=self.generator_lr*self.g_workers,betas=(0.5,0.999)),
        ...
        discriminator_optimizer = hvd.DistributedOptimizer(
            torch.optim.Adam(self.discriminator.parameters(),lr=self.discriminator_lr*self.d_workers,betas=(0.5,0.999)),
            backward_passes_per_step=self.num_backward_passes,
            named_parameters=self.discriminator.named_parameters()
        )

        # Broadcast the parameters from each network:
        hvd.broadcast_parameters(self.generator.state_dict(), root_rank=self.g_workers[0])
        hvd.broadcast_parameters(self.discriminator.state_dict(), root_rank=self.d_workers[0])
        ...
        for epoch in range(self.start_epoch,1+self.num_epochs):
            ...
            if epoch > 0:
               ...
               noise = torch.normal(self.noise_mean, self.noise_std, size=(self.batch_size,self.noise_dim),device=self.g_workers)
    ...
    def snaphsot_analysis(self):
        ...
        for epoch in range(self.start_epoch,1+self.num_epochs):
            if epoch % self.snapshot_epoch == 0:
                ...
                current_params, current_gen_data, current_residuals = self.hvd_performance_monitor.watch_residual_information_per_batch(real_data=monitoring_data_chunk)
    ...
    def post_training_analysis(self,n_samples):
        ...
        # Plot losses and accuracy as a function of the training epoch:
          ...
          axt.plot(self.hvd_training_time.numpy(),self.hvd_disc_real_loss.numpy(),'k--',linewidth=2.0,label='Disc. Real')
          axt.plot(self.hvd_training_time.numpy(),self.hvd_disc_fake_loss.numpy(),'r-.',linewidth=2.0,label='Disc. Fake')
          axt.plot(self.hvd_training_time.numpy(),[1.0]*self.hvd_training_time.numpy().shape[0],'m-',linewidth=2.0,label='Ideal: ' + r'$\sim 1.0$')
          axt.grid(True)
