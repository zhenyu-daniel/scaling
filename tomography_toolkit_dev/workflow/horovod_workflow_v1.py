...
from tomography_toolkit_dev.utils.data_science_mon.torch_ae_performance_monitor import AE_Performance_Monitor
from tomography_toolkit_dev.utils.data_science_mon.torch_ae_performance_plotter import AE_Performance_Plotter
...
class hvd_ae_workflow(Workflow):
    """
    Class to run the entire AE training workflow uisng horovod. The core function .run() has been divided into 6 sub-routines, in order to make this class a bit more accesible
    ...
    def __init__(self, module_names,
        ...
        self.num_backward_passes = self.read_settings_from_config("num_backward_passes",training_config,1)
        self.batch_size = self.read_settings_from_config("batch_size",training_config,64)
        self.n_events_to_analyze = self.read_settings_from_config("n_events_to_analyze",training_config,-1)
        self.num_epochs = self.read_settings_from_config("num_epochs",training_config,100000)
        self.print_info_epoch = self.read_settings_from_config("print_info_epoch",training_config,1000)
        self.read_performance_epoch = self.read_settings_from_config("read_performance_epoch",training_config,1000)
        self.n_final_analysis_samples = self.read_settings_from_config("n_final_analysis_samples",training_config,50000)
        self.snapshot_epoch = self.read_settings_from_config("snapshot_epoch",training_config,0)
        self.n_snapshot_samples = self.read_settings_from_config("n_snapshot_samples",training_config,5000)
        ...
        self.num_events_per_parameters = self.read_settings_from_config("num_events_per_parameter_set", training_config, 1)
        ...
        self.output_loc = self.read_settings_from_config("output_loc",training_config,"")
        self.result_folder = self.read_settings_from_config("result_folder",training_config,"results_AE_training")
        ...
        self.observable_names = self.read_settings_from_config("observable_names",data_config,[str(k) for k in range(1,1+generator_config['input_size'])])

        # Name of this workflow
        self.name = "horovod_workflow_v1"
        ...
        # Read in arguments for the name and the number of events to analyze:
        if len(sys.argv) >= 5:
            self.result_folder = sys.argv[1]
            self.n_events_to_analyze = int(sys.argv[2])
            self.batch_size = int(sys.argv[3])
            self.n_final_analysis_samples = int(sys.argv[4])
            self.n_snapshot_samples = int(sys.argv[5])
    ...
    def load_modules(self):
        ...
        self.module_chain_excluding_gen = [self.theory,self.experiment,self.filter]
    ...
    def load_tools(self):
        ...
        self.performance_plotter = AE_Performance_Plotter(config=ae_performance_plotter_cfg)
    ...
    def handle_output_data_storage(self):
        ...
        self.main_dir = self.out_dat_handler.create_output_data_folder(folder_name=self.result_folder+main_dir_extension,top_level_dir=self.output_loc)
        ...
        if self.store_results_as_npy == True:
    ...
    def fit(self):
        ...
        self.hvd_performance_monitor = AE_Performance_Monitor(
        ...
        # Get the time per epoch:
        training_time = []
        t_start = time.time()

        # TEST: Replace own data sampler with torch dataloader:
       # train_sampler = torch.utils.data.distributed.DistributedSampler(self.training_data,num_replicas=hvd.size(),rank=hvd.rank())
       # train_loader_list = list(torch.utils.data.DataLoader(self.training_data,batch_size=self.batch_size*self.num_events_per_parameters, sampler=train_sampler))
        ...
        for epoch in range(1,1+self.num_epochs):
            ...
            real_events = self.data_parser.get_random_batch_from_data(data=self.training_data,batch_size=self.batch_size)
            ref_events = self.data_parser.get_random_batch_from_data(data=self.training_data,batch_size=self.batch_size)
            # real_events = random.choice(train_loader_list).to(self.devices) 
            # ref_events = random.choice(train_loader_list).to(self.devices) 

            # while ref_events.size()[0] != real_events.size()[0]:
            #     real_events = random.choice(train_loader_list).to(self.devices) 
            #     ref_events = random.choice(train_loader_list).to(self.devices)
          
            # Train the encoder network:
            g_losses = self.generator.train(real_events,ref_events,self.norms,generator_optimizer)
            ...
            self.hvd_performance_monitor.watch_loss_and_dropout_rate_per_batch(
            ...
            if epoch % self.read_performance_epoch == 0:
                self.hvd_performance_monitor.collect_loss_and_dropout_rate_per_epoch()
            ...
            if epoch % self.print_info_epoch == 0 and hvd.rank() == 0:
                ...
                self.hvd_performance_monitor.print_loss_and_dropout_rate()
                print(" ")
            #----------------------------------------------------
        ...
        loss_dropout_rate_dict = self.hvd_performance_monitor.read_out_loss_and_dropout_rate()

        # Accumulated loss and dropout rate:
        self.hvd_gen_loss = hvd.allreduce(torch.as_tensor(loss_dropout_rate_dict['gen_loss']),'gen_loss')
        self.hvd_dropout_rate = hvd.allreduce(torch.as_tensor(loss_dropout_rate_dict['dropout_rate']),'dropout_rate')
    ...
    def snaphsot_analysis(self):
        ...
        for epoch in range(1,1+self.num_epochs):
            if epoch % self.snapshot_epoch == 0:
                ...
                current_params, current_gen_data, current_residuals = self.hvd_performance_monitor.predict_observables_and_residuals(real_data=monitoring_data_chunk)
                ...
                pdf_plots = self.physics_monitor.compare_true_and_generated_distributions(
                    ...
                    generated_npy_data = self.physics_monitor.create_gen_distributions(self.n_snapshot_samples,monitoring_data_chunk)
                )
    ...
    def post_training_analysis(self,n_samples):
        ...
        # Plot losses and accuracy as a function of the training epoch:
          loss_plots = self.performance_plotter.plot_loss_and_dropout_rate(
             gen_loss=self.hvd_gen_loss.numpy(), #--> Generator loss,
             dropout_rate=self.hvd_dropout_rate.numpy(),
             x_label = 'Training Epoch per ' + str(self.read_performance_epoch) #--> Set the x-label properly
          )

          fig_loss = loss_plots['loss_and_dropout_rate_plots'][0]
          fig_loss.savefig(self.data_science_dir + '/loss_and_dropout_rate.png')
          plt.close(fig_loss)

          # Check the loss as a function of time:
          figt,axt = plt.subplots(figsize=(12,8))

          axt.plot(self.hvd_training_time.numpy(),self.hvd_gen_loss.numpy(),'b-',linewidth=2.0)
          axt.set_xlabel('Time [s]')
          axt.set_ylabel('Encoder Training Loss')

          figt.savefig(self.data_science_dir + '/losses_vs_training_time.png')
          plt.close(figt)

          # Calculate actual residuals:
          params,generated_data, residuals = self.hvd_performance_monitor.predict_observables_and_residuals(real_data=self.analysis_data,n_samples=n_samples)
          ...
          generated_pdfs = self.physics_monitor.create_gen_distributions(n_samples,monitoring_data)
          ...
          if self.store_results_as_npy == True:
            ...
            np.save(self.data_science_npy_dir + '/dropout_rate.npy',self.hvd_dropout_rate.numpy())
