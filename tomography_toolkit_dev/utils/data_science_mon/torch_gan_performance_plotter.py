...
class GAN_Performance_Plotter(object):
    """
    This class is an extension of the GAN_Performance_Monitor. No complicated computation is done here. All functions defined within this class simply take numpy arrays
    ...
    #**********************************
    def plot_losses_and_accuracy(self,disc_real_loss,disc_fake_loss,gen_loss,disc_real_accuracy,disc_fake_accuracy,x_label="Training Epoch"):
        # Check the dimensionality:
        gen_loss = self.check_data_dimensionality(gen_loss)
        disc_real_loss = self.check_data_dimensionality(disc_real_loss)
        disc_fake_loss = self.check_data_dimensionality(disc_fake_loss)  
        disc_real_accuracy = self.check_data_dimensionality(disc_real_accuracy)
        disc_fake_accuracy = self.check_data_dimensionality(disc_fake_accuracy)
        ...
        fig.suptitle('GAN Training Results')
        fig.subplots_adjust(wspace=self.fig_wspace)

        x_values = [i for i in range(1,1+len(gen_loss))]

        # Plot the losses first:
        ax[0].plot(x_values,gen_loss,'b-',linewidth=self.line_width,label='Generator')
        ax[0].plot(x_values,disc_real_loss,'k--',linewidth=self.line_width,label='Disc. Real')
        ax[0].plot(x_values,disc_fake_loss,'r-.',linewidth=self.line_width,label='Disc. Fake')
        ax[0].plot([0,1+len(gen_loss)],[1.0,1.0],'m-',linewidth=self.line_width,label='Ideal: ' + r'$\sim 1.0$')
        ax[0].grid(True)
        ax[0].legend(fontsize=self.legend_font_size)
        ax[0].set_xlabel(x_label)
        ax[0].set_ylabel('Normalized Training Loss')

        # Now check out the accuracy:
        ax[1].plot(x_values,disc_real_accuracy,'k--',linewidth=self.line_width,label='Real')
        ax[1].plot(x_values,disc_fake_accuracy,'r-.',linewidth=self.line_width,label='Fake')
        ax[1].plot([0,1+len(gen_loss)],[0.5,0.5],'m-',linewidth=self.line_width,label='Ideal: ' + r'$\sim 0.5$')
        ax[1].grid(True)
        ax[1].legend(fontsize=self.legend_font_size)
        ax[1].set_xlabel(x_label)
        ax[1].set_ylabel('Discriminator Accuracy')
        ax[1].set_ylim(0.0,1.0)

        plot_dict = {'loss_and_accuracy_plots': [fig,ax]}

        plt.close()
        return plot_dict
    #**********************************

    # Plot information from residuals:

    #**********************************
    def plot_residual_information(self,residual_mean,residual_stddev,x_label="Training Epoch"):
        ...
        ax[0].plot(residual_mean,'b-',linewidth=self.line_width)
        ax[0].grid(True)
        ax[0].set_xlabel(x_label)
        ax[0].set_ylabel('Mean of Residual (Real Data - Fake Data)')

        ax[1].plot(residual_stddev,'b-',linewidth=self.line_width)
        ax[1].grid(True)
        ax[1].set_xlabel(x_label)
        ax[1].set_ylabel('Std. Dev. of Residual (Real Data - Fake Data)')

        plot_dict = {'residual_mean_and_stddev_plots': [fig,ax]}
        plt.close()
