#!/bin/bash -l                                                                                                            
#PBS -l select=2:ncpus=2:ngpus=2:system=polaris                                                                           
#PBS -l place=scatter                                                                                                     
...
# This script assumes that two nodes with max. 4 GPUs are available, thus we have to make sure
# that: (a) NGPUs <= 8 and: (b) NGPUsPerServer * 2 == NGPUs

if [ $NGPUs -le 8 ] && [ $(($NGPUsPerServer*2)) -eq $NGPUs ]
then
    ...
    serverSettings=$server1:$NGPUsPerServer,$server2:$NGPUsPerServer
   ...
   if [ $NGPUs -ge 8 ]
   then 
        echo "WARNING! Please make sure that: NGPUs <= 8"
   else 
        echo "WARNING! Please make sure that: NGPUsPerServer*2 == NGPUS"
    fi
