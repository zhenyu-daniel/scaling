...
resultFolder="proxy_gan_at_jlab"

# Define the output location where the result folder will be written to:
outputLoc="/work/data_science/dlersch/SCIDAC"
...
then
   ...
   then 
     ...
     horovodrun -np $NGPUs python $driverScript --batch_size $batchSize --result_folder $resultFolder --output_loc $outputLoc
     ...
     python $driverScript --batch_size $batchSize --result_folder $resultFolder --output_loc $outputLoc
